...
def generate_lines(out, shape, names, output_path, griding_num):
    h_samp = [115, 125, 135, 145, 155, 165, 175, 185, 195, 205, 215, 225, 235, 245, 255, 265,
    ...
    for n in range(out.shape[0]):
        out_j = out[n].data.cpu().numpy()
        cls = out_j[0:5,:,:]
        cls = np.argmax(cls, axis=0)
        offset = out_j[5,:,:]
        out_line = np.zeros((48,4))
        for i in range(48):
            for j in range(40):
                if cls[i,j]==1:
                    out_line[i,0]= int((j*41)+(offset[i,j]*41))
                if cls[i,j]==2:
                    out_line[i,1]= int((j*41)+(offset[i,j]*41))
                if cls[i,j]==3:
                    out_line[i,2]= int((j*41)+(offset[i,j]*41))
                if cls[i,j]==4:
                    out_line[i,3]= int((j*41)+(offset[i,j]*41))
        name = names[n]
        ...
        with open(line_save_path, 'w') as fp:
            for i in range(out_line.shape[1]):
                if np.sum(out_line[:, i] != 0) > 2:
                    for k in range(out_line.shape[0]):
                        if out_line[k, i] > 0:
                            fp.write('%d %d ' % (out_line[k,i], h_samp[k]))
                    fp.write('\n')
...
def run_test(net, data_root, exp_name, work_dir, griding_num, use_aux,distributed, batch_size=8):
    ...
    for i, data in enumerate(dist_tqdm(loader)):
        ...
        with torch.no_grad():
            out = net(imgs)
            out = out[2]
        generate_lines(out, imgs[0,0].shape, names, output_path, griding_num)
...
def run_test_tusimple(net,data_root,work_dir,exp_name,griding_num,use_aux, distributed,batch_size = 1):
